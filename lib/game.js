...
class Game {
  ...
  checkItemCollision() {
    if (this.Treat.x >= this.baller.x &&
      this.Treat.x <= this.baller.x + 80 &&
      this.Treat.y >= this.baller.y &&
      this.Treat.y <= this.baller.y + 80
    ) {

      if (this.Treat.type === "github") {
        this.score += 10;
      } else {
        this.score += 50;
      }
