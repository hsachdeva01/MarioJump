...
chooseChar(){
  ...
  showTimer() {
    ctx.fillStyle = "Black";
    ctx.font = "12pt mario";
    ctx.fillText("Timer:" + this.timer, 8, height-480);
  }
  ...
  checkEnemyCollision() {
    if (this.facing === "right") {
      if (this.enemy.centerX >= this.player.x + 15 &&
        this.enemy.centerX <= this.player.x + 64 &&
        this.enemy.centerY >= this.player.y + 10 &&
        this.enemy.centerY <= this.player.y + 70
      ) {
        return true;
      }
      return false;
